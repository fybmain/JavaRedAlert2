 ...
 * 步兵图标父类
 */
public class Tab02ConstIcon extends JLabel{

	private static final long serialVersionUID = 1L;
	public SoldierEnum soldierInfo = null;
	
	public final static int STATUS_IDLE = 0;//正常
	public final static int STATUS_TRAINING = 1;//生产小兵中  并且在倒计时
	 ...
	 * 自己的引用,给任务内部类使用
	 */
	public Tab02ConstIcon myself;
	
	/**
	 * 构造方法
	 */
	public Tab02ConstIcon(SoldierEnum soldierInfo) {
		this.soldierInfo = soldierInfo;
		List<ShapeUnitFrame> list = ShpResourceCenter.loadShpResource(soldierInfo.shpIconPrefix, "cameo", false);
		...
		this.setToolTipText("<html>"
				+ "<b>"
				+ "<div>"
				+ "<span>"+soldierInfo.soldierName+"</span><br/>"
				+ "<span>$"+soldierInfo.price+"</span>"
				+ "</div>"
				+ "</b></html>");
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if(e.getButton()==MouseEvent.BUTTON1) {//左键
					/*
					 * 小兵生产的队列、图标的状态  同一由管理器管理
					 */
					if(status==STATUS_IDLE) {
						if(Tab02Manager.trainList.isEmpty()) {
							Constructor.playOneMusic("ceva066");//Training
						}
						Tab02Manager.addTrainPlan(soldierInfo);
						return;
					}
				...
				if(e.getButton()==MouseEvent.BUTTON3) {//右键
					...
					if(status==STATUS_ON_HOLD) {//暂停转取消
						Tab02Manager.reduceTrainPlan(soldierInfo);
					...
					if(status==STATUS_TRAINING_WAIT) {//排队转取消
						Tab02Manager.reduceWaitingTrainPlan(soldierInfo);
						return;
					}
				}
			}
			...
			TimerTask refreshTask = new TimerTask() {
				...
				public void run() {
					...
					if(status==STATUS_TRAINING) {
						...
						//判断是否需要画数字
						int num = Tab02Manager.countNum(soldierInfo);
						if(num>1) {
							if(num<10) {
					...
					if(status==STATUS_ON_HOLD) {
						if(!isOnHoldFlag) {
							...
							if(num>1) {
								g2d.drawImage(dengdai, 0, 0 , null);
							}else {
